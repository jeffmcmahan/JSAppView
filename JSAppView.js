;(function () {
    ...
    /**
     * Report a value in the native iOS console.
     * @param {Array} vals
     */
    function log(vals) {
        vals.map(val => {
    ...
    window.onerror = function (messageOrEvent, source, lineno, colno, error) {
        console.log(`Error in ${source || '??'} at line ${lineno}.`, error)
    }
    ...
    function readFile(basename, encoding='utf8') {
        ...
        if (!['utf8', 'base64'].includes(encoding)) throw new Error(
            ...
            'the file directly with a file:// URL (use: JSAppViewFileSystem.getFileURL(basename)), '+
            'or use an XHR to read it from a file:// URL.'
        )
